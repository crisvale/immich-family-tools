...
**Prozess-Stand: v1.16.0** — Stand der Vorlage, aus der dieses Projekt stammt.
...
## Umgebungsaussagen — gemessen, nicht angenommen

Jede Aussage dieser Datei über die Umgebung ist eine **Behauptung** und trägt
deshalb ein Kommando mit Exit-Code. Der Anlass steht in `docs/agents/lehren.md`
§32: „Kein Node/npm lokal auf diesem Rechner" stand in fünf Projektständen der
Vorlage — auf dem Rechner lagen Node 24 und npm 11, ein Bauer baute sein Ritual
auf die Aussage, und das Ritual vernichtete Daten.

Gemessen am 22.09.2026 auf dem Entwicklungsrechner:

| Behauptung                | Kommando                                                                  | Ergebnis                |
| ------------------------- | ------------------------------------------------------------------------- | ----------------------- |
| Node liegt lokal          | `node --version`                                                          | Exit 0, `v24.18.0`      |
| npm liegt lokal           | `npm --version`                                                           | Exit 0, `11.16.0`       |
| Python liegt lokal        | `python --version`                                                        | Exit 0, `Python 3.14.5` |
| Docker liegt lokal        | `docker --version`                                                        | Exit 0, `29.6.1`        |
| `gh` liegt lokal          | `gh --version`                                                            | Exit 0, `2.93.0`        |
| kein Migrations-Framework | `grep -rniE 'alembic\|sqlalchemy\|psycopg' backend/requirements*.txt`     | Exit 1 (kein Treffer)   |
| kein Datenbank-Dienst     | `grep -cE 'image: *(postgres\|mysql\|mariadb\|mongo)' docker-compose.yml` | `0`, Exit 1             |

**Die eine Stelle, an der lokal und CI auseinandergehen:** Lokal läuft **npm
11** (Node 24), die CI fährt **Node 22** (`ci.yml`, `node-version: "22"`) und
damit npm 10. Ein `package-lock.json`, das npm 11 erzeugt, kann npm 10
zurückweisen — der Lauf scheitert dann in der CI und nirgends vorher. Deshalb
entstehen Änderungen an der Sperrdatei mit `npx -y npm@10`, nicht mit dem
lokalen npm. Die Zahlen hier veralten mit dem nächsten Node-Update; wer sie
liest, misst nach, statt sie zu glauben.
...
| _R1_         | **keine frei vergebbare Klasse** — seit v1.15.0 in der Zeile „Nacharbeit" aufgegangen, die für JEDE Nacharbeit gilt, nicht nur für mechanische Auflagen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | siehe „Nacharbeit"                                                                                                                                                                                                                                                                                   |
| **R2**       | alles ohne R0-, R3- oder R4-Auslöser (der Normalfall)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Blindprüfer + Fremdprüfer                                                                                                                                                                                                                                                                            |
| **R3**       | Datenmigration — unabhängig davon, ob ein Migrations-Framework beteiligt ist: Gemeint ist der VORGANG, bestehende Daten werden unumkehrbar umgewandelt (auch eine selbstheilende JSON-Schema-Wanderung ist eine); Berechtigungs-/Datenschutzlogik; Geld/Steuern; Außenwirkung über eine Schnittstelle; **Fremdcode** — Produktionscode, den ein FREMDES System beigesteuert hat (Patch eines Anbietermodells, zugelieferter Zweig, übernommener Schnipsel) — **und fremde Werkzeug-Mechanik, die in den eigenen Prozess übernommen wird** (Hooks, Agenten-Definitionen, Skills eines anderen Frameworks: je Zusicherung wie „nur lesend" oder „schlägt geschlossen fehl" eine negative Probe); **kritische Dateien** — Wächter, Gate-Probe, Selbstprobe, Hook-Einträge, Gate-, Release- und Prüfskripte (Liste in `docs/agents/panel.md`, „Kritische Dateien"), unabhängig von der Diffgröße — **zwei Ausnahmen, beide nur für das Panel, die Probe durch die echte Tür bleibt in beiden Fällen:** (a) byte-identische Übernahme aus der Vorlage beim Abgleich (`docs/agents/abgleich.md`, Absatz „Panel für den Abgleich-Commit"); (b) **reine Kopftext-Änderung** — nur Kommentare oder Docstrings, Syntaxbaum nachweislich gleich; dann R2. **Bedingungen, Verfahren und Beleg stehen in `docs/agents/panel.md`, „Kritische Dateien"** und werden hier nicht wiederholt (Owner-Entscheid 2026-09-21). **NICHT gemeint: der eigene Bau-Subagent** — sonst ist der Auslöser immer erfüllt und R2 verschwindet | volles Panel mit Gegenprüfer + risikospezifische Probe durch die echte Tür                                                                                                                                                                                                                           |
| _Nacharbeit_ | jede Nacharbeitsrunde nach einem Panel, gleich welcher Klasse; **höchstens zwei** (gezählt: Erstrunde, Nacharbeit 1, Nacharbeit 2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Blindprüfer; Gegenprüfer nur bei neuer Tür; bei R3 und R4 zusätzlich die risikospezifische Probe durch den Arbiter. **Nach Nacharbeit 2 wird nicht weiter gebaut:** offene oder neue BLOCKER → anhalten, Owner fragen; nur noch Nicht-Blocker → landen, die Funde werden ein eigenes Issue mit Beleg |
...
**Kostenschwelle für die Fremdprüfer-Ersatzregel: keine** — Owner-Entscheid
19.09.2026, pauschal freigegeben; die Größenordnung liegt bei Cent je Lauf.
Bei **Guthabenende (HTTP 402)** wird nicht ausgeliefert und nicht still
verkürzt, sondern der Owner gefragt. Beim Erreichen der Schwelle gilt dasselbe wie bei erschöpftem
Guthaben — Regel und Verfahren stehen nur in `docs/agents/panel.md`,
Klumpenrisiko Punkt 4; die Form des geordneten Abbruchs steht dort unter
„Ein Kostenstopp steht fest, BEVOR die Serie beginnt".

Bis zu drei Stimmen mit festen Namen — wie viele ein Slice bekommt, sagt die
Risiko-Tabelle oben; Details und Kommandos in `docs/agents/panel.md`:

1. **Blindprüfer** — frischer Reviewer-Subagent, bekommt **nur Diff und
   Repo**, nicht den Bau-Brief. Liefert erfahrungsgemäß die schwersten Funde,
   weil er den Code liest statt der Absicht.
2. **Fremdprüfer** — Modell eines anderen Anbieters über denselben Diff.
3. **Gegenprüfer** — zweite frische Claude-Stimme mit Widerlegungsauftrag;
   Pflicht bei R3, optional bei R2 (bei R2 alternativ eine günstige
   diff-only-Fremdstimme). Bis v1.14 hieß er „Zweitblind-Stimme", im
   Pilot, aus dem er stammt, „Arm C".

**Nacharbeit** hat eine eigene Zeile in der Risiko-Tabelle oben; dort steht die
Schwelle, in `panel.md`, Schritt 4, stehen Verfahren und Beleg.

Der Panel-Kommentar hat eine **feste Form**: eine Überschrift je Stimme, auch
bei „keine Funde". Fällt eine Stimme aus, steht unter ihrer Überschrift der
**Grund** und der Schritt, nach dem gehandelt wurde. **Die Schritte selbst
stehen nur in `docs/agents/panel.md`, „Wenn eine Stimme ausfällt"** — vier
Schritte, mit den Sonderfällen Guthabenende und Token-Budget; diese Datei
wiederholt sie nicht. Ein Slice ohne vollständiges oder vermerkt-verkürztes
Panel gilt als **nicht geprüft**.
**Verkürzen** (eine Stimme bewusst weglassen) nur mit Grund unter der
Überschrift der weggelassenen Stimme — und nie unter die **Untergrenze**: bei
R2 der Blindprüfer; bei R3 und R4 zusätzlich nie ohne Gegenprüfer und
Tür-Probe. Die Nacharbeit folgt ihrer eigenen Zeile in der Tabelle.
**Ein AUSFALL einer Pflichtstimme ist keine vermerkte Verkürzung:** Verkürzt
heißt „bewusst und begründet weggelassen"; eine ausgefallene Pflichtstimme wird
vor der Auslieferung nachgeholt, oder der Owner gibt die Auslieferung ohne sie
ausdrücklich frei — beides steht im Panel-Kommentar. **Dasselbe gilt für eine
teilweise ausgefallene Tür-Probe** bei R3 und R4 (unter der Überschrift
„Tür-Probe (R3/R4)" in `docs/agents/panel-kommentar.md` vermerkt): Sie gehört zur
Untergrenze, also nachholen oder ausdrückliche Owner-Freigabe.

**Modellnamen und Aufrufkommandos der Stimmen für dieses Projekt** stehen in
`docs/agents/panel.md`, Abschnitt „Eingetragen: unsere Aufrufe" — hier nicht
wiederholt, um Doppelpflege zu vermeiden.
